...
c      kelem=17950000
      ...
      write(*,*) 'memory needed = ',kna3,' in prgram ecgmc'
      stop 55555
      endif
      knb0=1
      knb1=knb1+knb0
      ...
      knc8=knc8+knc7
      knc9=knc9+knc8
      if (knc9-1.gt.35900000) then
      write(*,*) 'exceed memory of array ic'
      write(*,*) 'memory of ic = 35900000'
      write(*,*) 'memory needed = ',knc9,' in prgram ecgmc'
      stop 55555
      endif
     ...
     *eun(kdgof,knode),eum(kdgof,knode),eu(kdgof,knode),
     *
...
cc      write(*,7) ((u(j,i),j=1,kdgof),i=1,knode)
      ...
      open(12,file='unodb',form='unformatted',status='old')
      else
      open(11,file='unod',form='unformatted',status='old')
      open(12,file='unodb',form='unformatted',status='old')
      endif
      read(11) ((eun(j,i),i=1,knode),j=1,kdgof)
      read(12) ((eum(j,i),i=1,knode),j=1,kdgof)
...
c       check solution
     ...
     & ityp,ncoor,num,time,dt,neq,neq1,maxt,maxa,nodvar,coor,node,emate,
     & a,na,numcol,
     *es,em,ef,estifn,estifv,eun,eum,eu,
     *
     *u)
      implicit real*8 (a-h,o-z)
     ...
     *eum(kdgof,knode),eu(kdgof,knode),estifn(k,k),estifv(kk),
     *
...
18      format (1x,8e9.2)
	...
	jnod = node((ne-1)*nnode+j)
      ...
      coef(j+(l-1+1*kdgof)*nne)=eum(l,jnod)
110   continue
	do 120 i=1,ncoor
	nr=nr+1
