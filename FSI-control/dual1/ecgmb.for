...
c      kelem=17950000
      ...
      write(*,*) 'memory needed = ',kna3,' in prgram ecgmb'
      stop 55555
      endif
      knb0=1
      knb1=knb1+knb0
      ...
      write(*,*) 'memory needed = ',knb2,' in prgram ecgmb'
      stop 55555
      endif
      knc0=1
      knc1=knc1+knc0
      ...
      knc13=knc13+knc12
      knc14=knc14+knc13
      if (knc14-1.gt.35900000) then
      write(*,*) 'exceed memory of array ic'
      write(*,*) 'memory of ic = 35900000'
      write(*,*) 'memory needed = ',knc14,' in prgram ecgmb'
      stop 55555
      endif
     ...
     *eu0(kdgof,knode),eu(kdgof,knode),eun(knode),evn(knode),
     *eus(knode),evs(knode),euh(knode),evh(knode),
     *
...
cc      write(*,7) ((u(j,i),j=1,kdgof),i=1,knode)
      ...
      numtyp = 2
      if (it.eq.0) then
      open(11,file='what',form='unformatted',status='old')
      open(12,file='unod',form='unformatted',status='old')
      open(13,file='unods',form='unformatted',status='old')
      ...
      read(12) (eun(i),i=1,knode),
     &  (evn(i),i=1,knode)
      read(13) (eus(i),i=1,knode),
     &  (evs(i),i=1,knode)
      read(14) (euh(i),i=1,knode),
     &  (evh(i),i=1,knode)
...
115     continue
     ...
     &eun,evn,eus,evs,euh,evh,eu0,eu,
     & u)
2000    continue
...
c       check solution
     ...
     & ityp,ncoor,num,time,dt,neq,neq1,maxt,maxa,nodvar,coor,node,emate,
     & a,na,numcol,
     *es,em,ef,estifn,estifv,eun,evn,eus,
     *evs,euh,evh,eu0,eu,
     *u)
      implicit real*8 (a-h,o-z)
     ...
     *evn(knode),eus(knode),evs(knode),euh(knode),
     *evh(knode),eu0(kdgof,knode),eu(kdgof,knode),
...
18      format (1x,8e9.2)
	...
	jnod = node((ne-1)*nnode+j)
      ...
      coef(j+4*nne)=euh(jnod)
      coef(j+5*nne)=evh(jnod)
	do 120 i=1,ncoor
	nr=nr+1
...
140	prmt(j) = emate((imate-1)*nmate+j)
	...
	prmt(nmate+2)=dt
      ...
      goto (1,2), ityp
1     call beq9g3(r,coef,prmt,es,em,ec,ef,ne)
      goto 3
2     call bgl3g3(r,coef,prmt,es,em,ec,ef,ne)
      goto 3
3     continue
