...
c      kelem=17950000
      ...
      knc17=knc17+knc16
      knc18=knc18+knc17
      if (knc18-1.gt.35900000) then
      write(*,*) 'exceed memory of array ic'
      write(*,*) 'memory of ic = 35900000'
      write(*,*) 'memory needed = ',knc18,' in prgram ecgma'
      stop 55555
      endif
     ...
     *ic(knc8),ic(knc9),ic(knc10),ic(knc11),ic(knc12),
     *ic(knc13),ic(knc14),ic(knc15),ic(knc16),ic(knc17),
     *filename)
      end
     ...
     *eu0,eu,eus,evs,eun,evn,
     *eum,evm,eud,evd,euw,evw,
     *sml,
     *filename)
      implicit real*8 (a-h,o-z)
     ...
     *eun(knode),evn(knode),eum(knode),evm(knode),
     *eud(knode),evd(knode),euw(knode),evw(knode),
     *
...
cc      write(*,7) ((u(j,i),j=1,kdgof),i=1,knode)
      ...
      open(11,file='uhat',form='unformatted',status='old')
      open(12,file='unods',form='unformatted',status='old')
      open(13,file='unod',form='unformatted',status='old')
      open(14,file='unodb',form='unformatted',status='old')
      open(15,file='what',form='unformatted',status='old')
      open(16,file='ugoal',form='unformatted',status='old')
      read(11) ((eu0(j,i),i=1,knode),j=1,kdgof-1)
      read(12) (eus(i),i=1,knode),
     &  (evs(i),i=1,knode)
      read(13) (eun(i),i=1,knode),
     &  (evn(i),i=1,knode)
      read(14) (eum(i),i=1,knode),
     &  (evm(i),i=1,knode)
      read(15) (euw(i),i=1,knode),
     &  (evw(i),i=1,knode)
      read(16) (eud(i),i=1,knode),
     &  (evd(i),i=1,knode)
      close(11)
      close(12)
      close(13)
...
cccccccccccccccccccccccccccccccccccccc	  
      ...
      open(16,file='press',form='unformatted',status='unknown')
      read(16)  (eu0(kdgof,i),i=1,knode)
      close(16)
c.......open diag file
      open (2,file=' ',form='unformatted',status='old')
...
115     continue
     ...
     &eus,evs,eun,evn,eum,evm,eud,evd,euw,evw,
     &eu0,eu,
     & u)
2000    continue
...
c       check solution
     ...
     & ityp,ncoor,num,time,dt,neq,neq1,maxt,maxa,nodvar,coor,node,emate,
     & a,na,numcol,
     *es,em,ef,estifn,estifv,eus,evs,eun,
     *evn,eum,evm,eud,evd,euw,evw,eu0,
     *eu,
     *u)
      implicit real*8 (a-h,o-z)
     ...
     *evs(knode),eun(knode),evn(knode),eum(knode),
     *evm(knode),eud(knode),evd(knode),euw(knode),
     *evw(knode),eu0(kdgof,knode),eu(kdgof,knode),
...
18      format (1x,8e9.2)
	...
	jnod = node((ne-1)*nnode+j)
      ...
      coef(j+2*nne)=eus(jnod)
      coef(j+3*nne)=evs(jnod)
      coef(j+4*nne)=eum(jnod)
      coef(j+5*nne)=evm(jnod)
      coef(j+6*nne)=eud(jnod)
      coef(j+7*nne)=evd(jnod)
      coef(j+8*nne)=euw(jnod)
      coef(j+9*nne)=evw(jnod)
	do 120 i=1,ncoor
	nr=nr+1
...
1     call aeq9g3(r,coef,prmt,es,em,ec,ef,ne)
      goto 3
2     call agl3g3(r,coef,prmt,es,em,ec,ef,ne)
      goto 3
3     continue
