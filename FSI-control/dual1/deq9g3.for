...
c .... coefr ---- nodal coef value
     ...
     & eload(9)
      dimension prmt(*),coef(6),coefr(9,6),coorr(2,9),coor(2)
      common /rdeq9g3/ro(9,27),
     & co(9,3)
c .... store shape functions and their partial derivatives
c .... for all integral points
      common /vdeq9g3/rctr(2,2),crtr(2,2),coefd(6,5),coefc(6,5)
...
c .... the following is the mass matrix computation
      ...
      do 301 i=1,nvard(1)
      iv = kvord(i,1)
      emass(iv)=emass(iv)+elump(iv)*co(i,1)
301   continue
...
c .... by reference coordinate value
      ...
      common /ccdeq9g3/ x(9),y(9),un(9),vn(9),us(9),vs(9),
     &ud(9),vd(9)
      external ftdeq9g3
      do 100 n=1,9
      x(n)=coorr(1,n)
...
c .... by reference coordinate value
      ...
      rx=refc(1)
      ry=refc(2)
      call dcoef(fedeq9g3,refc,coef,coefd,2,6,2)
...
c .... coef function caculation
      implicit real*8 (a-h,o-z)
      dimension refc(2)
      common /ccdeq9g3/ xa(9),ya(9),un(9),vn(9),us(9),vs(9),
     &ud(9),vd(9)
      common /vdeq9g3/ rctr(2,2),crtr(2,2),coefd(6,5),coefc(6,5)
      common /coord/ coor(3),coora(27,3)
      x=coor(1)
...
2     fedeq9g3=+(+rx*(+rx-1.)/2.*ry*(+ry-1.)/2.)*vn(1)+(+
     ...
     & ry*(+1.+ry)/2.)*vn(3)
      goto 1000
3     fedeq9g3=+(+rx*(+rx-1.)/2.*ry*(+ry-1.)/2.)*us(1)+(+
     ...
     & ry*(+1.+ry)/2.)*us(3)
      goto 1000
4     fedeq9g3=+(+rx*(+rx-1.)/2.*ry*(+ry-1.)/2.)*vs(1)+(+
     ...
     & ry*(+1.+ry)/2.)*vs(3)
      goto 1000
5     fedeq9g3=+(+rx*(+rx-1.)/2.*ry*(+ry-1.)/2.)*ud(1)+(+
     ...
     & ry*(+1.+ry)/2.)*ud(3)
      goto 1000
6     fedeq9g3=+(+rx*(+rx-1.)/2.*ry*(+ry-1.)/2.)*vd(1)+(+
