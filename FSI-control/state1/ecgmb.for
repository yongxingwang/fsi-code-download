...
c      kelem=17950000
      ...
      knc7=knc7+knc6
      knc8=knc8+knc7
      if (knc8-1.gt.35900000) then
      write(*,*) 'exceed memory of array ic'
      write(*,*) 'memory of ic = 35900000'
      write(*,*) 'memory needed = ',knc8,' in prgram ecgmb'
      stop 55555
      endif
...
cc      write(*,7) ((u(j,i),j=1,kdgof),i=1,knode)
      ...
      numtyp = 1
      if (it.eq.0) then
      open(11,file='unodb',form='unformatted',status='old')
      else
      open(11,file='unodb',form='unformatted',status='old')
      endif
...
cccccccccccccccccccccccccccccccc
      ...
      read(10)  ((eu(j,i),i=1,knode),j=1,kdgof)
      close(10)
  
c.......open id file
      open (1,file='idb0',form='unformatted',status='old')
      ...
      do j=1,knode
        ...
        if (nodvar(i,j).eq.-2) u(i,j)=eu(i,j)
				
c        if (nodvar(i,j).eq.-3) u(i,j)=eu(i,6942)

c        if (nodvar(i,j).eq.-4) u(i,j)=eu(i,6928)	
