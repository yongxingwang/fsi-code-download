...
c .... for all integral points
      common /vaeq9g3/rctr(2,2),crtr(2,2),coefd(4,5),coefc(4,5)
...
106   continue
      dsxx = +coefc(1,1)+coefc(1,1)
      dsxy = +coefc(1,2)+coefc(2,1)
      dsyx = +coefc(2,1)+coefc(1,2)
      dsyy = +coefc(2,2)+coefc(2,2)
      ddxx = +coefc(1,1)*coefc(1,1)
     * +coefc(2,1)*coefc(2,1)
      ddxy = +coefc(1,1)*coefc(1,2)
     * +coefc(2,1)*coefc(2,2)
      ddyx = +coefc(1,2)*coefc(1,1)
     * +coefc(2,2)*coefc(2,1)
      ddyy = +coefc(1,2)*coefc(1,2)
     * +coefc(2,2)*coefc(2,2)
c .... the following is the stiffness computation
      ...
      do 201 j=1,9
      jv=kvord(j,1)
      stif=-cu(i,2)*cu(j,2)*coefc(1,1)*c1*dt
     & -cu(i,3)*cu(j,3)*coefc(1,1)*c1*dt
     & -cu(i,2)*cu(j,2)*coefc(1,1)*c1*dt
     & -cu(i,2)*cu(j,3)*coefc(1,2)*c1*dt
...
202    continue
      ...
      do 203 j=1,9
      jv=kvord(j,2)
      stif=-cu(i,2)*cv(j,2)*coefc(1,2)*c1*dt
     & -cu(i,3)*cv(j,3)*coefc(1,2)*c1*dt
     & -cu(i,3)*cv(j,2)*coefc(1,1)*c1*dt
     & -cu(i,3)*cv(j,3)*coefc(1,2)*c1*dt
      estif(iv,jv)=estif(iv,jv)+stif*weigh
203    continue
204    continue
      ...
      do 205 j=1,9
      jv=kvord(j,1)
      stif=-cv(i,2)*cu(j,2)*coefc(2,1)*c1*dt
     & -cv(i,3)*cu(j,3)*coefc(2,1)*c1*dt
     & -cv(i,2)*cu(j,2)*coefc(2,1)*c1*dt
     & -cv(i,2)*cu(j,3)*coefc(2,2)*c1*dt
      estif(iv,jv)=estif(iv,jv)+stif*weigh
205    continue
206    continue
      do 208 i=1,9
      ...
      do 207 j=1,9
      jv=kvord(j,2)
      stif=-cv(i,2)*cv(j,2)*coefc(2,2)*c1*dt
     & -cv(i,3)*cv(j,3)*coefc(2,2)*c1*dt
     & -cv(i,3)*cv(j,2)*coefc(2,1)*c1*dt
     & -cv(i,3)*cv(j,3)*coefc(2,2)*c1*dt
      estif(iv,jv)=estif(iv,jv)+stif*weigh
207    continue
208    continue
...
c .... shape function caculation
      implicit real*8 (a-h,o-z)
      common /ccaeq9g3/ xa(9),ya(9),usa(9),vsa(9),
     &fxa(9),fya(9)
      common /vaeq9g3/ rctr(2,2),crtr(2,2),coefd(4,5),coefc(4,5)
      dimension refc(2)
...
c .... by reference coordinate value
      ...
      common /ccaeq9g3/ x(9),y(9),us(9),vs(9),fx(9),fy(9)
      external ftaeq9g3
      do 100 n=1,9
      x(n)=coorr(1,n)
      y(n)=coorr(2,n)
...
c .... by reference coordinate value
      ...
      rx=refc(1)
      ry=refc(2)
      call dcoef(feaeq9g3,refc,coef,coefd,2,4,2)
...
c .... coef function caculation
      implicit real*8 (a-h,o-z)
      dimension refc(2)
      common /ccaeq9g3/ xa(9),ya(9),us(9),vs(9),fx(9),fy(9)
      common /vaeq9g3/ rctr(2,2),crtr(2,2),coefd(4,5),coefc(4,5)
      common /coord/ coor(3),coora(27,3)
      x=coor(1)
      y=coor(2)
      ...
      ry=refc(2)
      goto (1,2,3,4) n
1     feaeq9g3=+(+rx*(+rx-1.)/2.*ry*(+ry-1.)/2.)*us(1)+(+
     ...
     & ry*(+1.+ry)/2.)*us(3)
      goto 1000
2     feaeq9g3=+(+rx*(+rx-1.)/2.*ry*(+ry-1.)/2.)*vs(1)+(+
     ...
     & ry*(+1.+ry)/2.)*vs(3)
      goto 1000
3     feaeq9g3=+(+rx*(+rx-1.)/2.*ry*(+ry-1.)/2.)*fx(1)+(+
     & (+1.-rx**2)*ry*(+ry-1.)/2.)*fx(5)+(+rx*(+1.+rx)/2.*ry*
     & (+ry-1.)/2.)*fx(2)+(+rx*(+rx-1.)/2.*(+1.-ry**2))*fx(8)
     & +(+(+1.-rx**2)*(+1.-ry**2))*fx(9)+(+rx*(+1.+rx)/2.*(+
     & 1.-ry**2))*fx(6)+(+rx*(+rx-1.)/2.*ry*(+1.+ry)/2.)*fx(4)
     & +(+(+1.-rx**2)*ry*(+1.+ry)/2.)*fx(7)+(+rx*(+1.+rx)/2.*
     & ry*(+1.+ry)/2.)*fx(3)
      goto 1000
4     feaeq9g3=+(+rx*(+rx-1.)/2.*ry*(+ry-1.)/2.)*fy(1)+(+
     & (+1.-rx**2)*ry*(+ry-1.)/2.)*fy(5)+(+rx*(+1.+rx)/2.*ry*
     & (+ry-1.)/2.)*fy(2)+(+rx*(+rx-1.)/2.*(+1.-ry**2))*fy(8)
     & +(+(+1.-rx**2)*(+1.-ry**2))*fy(9)+(+rx*(+1.+rx)/2.*(+
     & 1.-ry**2))*fy(6)+(+rx*(+rx-1.)/2.*ry*(+1.+ry)/2.)*fy(4)
     & +(+(+1.-rx**2)*ry*(+1.+ry)/2.)*fy(7)+(+rx*(+1.+rx)/2.*
     & ry*(+1.+ry)/2.)*fy(3)
      goto 1000
1000  return
      end
