...
c .... coefr ---- nodal coef value
     ...
     & eload(6)
      ...
      common /rdll3g3/ru(3,6),rv(3,6),
     & cu(3,2),cv(3,2)
c .... store shape functions and their partial derivatives
c .... for all integral points
      common /vdll3g3/rctr(1,1),crtr(1,1),coefd(2,2),coefc(2,2)
      common /ddll3g3/ refc(1,3),gaus(3),
...
10    continue
      ...
      call dll3g3t(nnode,nrefc,ncoor,refc(1,igaus),coor,coorr,
     & rctr,crtr,det,coefr)
...
c .... crtr ---- inverse matrix of Jacobi's matrix
      x=coor(1)
      rx=refc(1,igaus)
      call edll3g3(refc(1,igaus),coef,coorr,coefr,coefd)
c .... compute coef functions and their partial derivatives
      iu=(igaus-1)*2+1
      iv=(igaus-1)*2+1
      if (num.gt.1) goto 2
c .... the following is the shape function caculation
      call dll3g31(refc(1,igaus),ru(1,iu),rctr,crtr)
      call dll3g32(refc(1,igaus),rv(1,iv),rctr,crtr)
2     continue
...
8     format(1x,6f12.3)
      ...
      subroutine dll3g31(refc,shpr,rctr,crtr)
...
c .... shapr ---- store shape functions and their partial derivatives
      ...
      external fdll3g31
      rx=refc(1)
      call dshap(fdll3g31,refc,shpr,1,3,1)
...
c .... which is in the file ccshap.for of FEPG library
      ...
      real*8 function fdll3g31(refc,n)
c .... shape function caculation
      implicit real*8 (a-h,o-z)
      common /ccdll3g3/ xa(3),una(3),vna(3)
      common /vdll3g3/ rctr(1,1),crtr(1,1),coefd(2,2),coefc(2,2)
      dimension refc(1)
      ...
      rx=refc(1)
      goto (1,2,3) n
1     fdll3g31=+rx*(+rx-1.)/2. 
      goto 1000
2     fdll3g31=+(+1.-rx**2) 
      goto 1000
3     fdll3g31=+rx*(+1.+rx)/2. 
      goto 1000
1000  return
      end

      subroutine dll3g32(refc,shpr,rctr,crtr)
c .... compute shape functions and their partial derivatives
c .... shapr ---- store shape functions and their partial derivatives
      implicit real*8 (a-h,o-z)
      dimension refc(1),shpr(3,2),rctr(1,1),crtr(1,1)
      external fdll3g32
      rx=refc(1)
      call dshap(fdll3g32,refc,shpr,1,3,1)
c .... shape function and their derivatives computation
c .... compute partial derivatives by centered difference
c .... which is in the file ccshap.for of FEPG library
      return
      ...
      real*8 function fdll3g32(refc,n)
c .... shape function caculation
      implicit real*8 (a-h,o-z)
      common /ccdll3g3/ xa(3),una(3),vna(3)
      common /vdll3g3/ rctr(1,1),crtr(1,1),coefd(2,2),coefc(2,2)
      ...
      rx=refc(1)
      goto (1,2,3) n
1     fdll3g32=+rx*(+rx-1.)/2. 
      goto 1000
2     fdll3g32=+(+1.-rx**2) 
      goto 1000
3     fdll3g32=+rx*(+1.+rx)/2. 
      goto 1000
1000  return
      end

      subroutine tdll3g3(refc,coor,coorr,coefr,rc)
...
200   continue
      rx=refc(1)
      call dcoor(ftdll3g3,refc,coor,rc,1,1,1)
...
c .... which is in the file ccshap.for of FEPG library
      ...
      real*8 function ftdll3g3(refc,n)
c .... coordinate transfer function caculation
      ...
      rx=refc(1)
      goto (1) n
1     ftdll3g3=+(+rx*(+rx-1.)/2.)*x(1)+(+(+1.-rx**2))*x(3)
     & +(+rx*(+1.+rx)/2.)*x(2)
      goto 1000
1000  return
      end

      subroutine edll3g3(refc,coef,coorr,coefr,coefd)
...
c .... by reference coordinate value
      ...
      external fedll3g3
      rx=refc(1)
      call dcoef(fedll3g3,refc,coef,coefd,1,2,2)
...
c .... which is in the file ccshap.for of FEPG library
      ...
      real*8 function fedll3g3(refc,n)
c .... coef function caculation
      implicit real*8 (a-h,o-z)
      dimension refc(1)
      common /ccdll3g3/ xa(3),un(3),vn(3)
      common /vdll3g3/ rctr(1,1),crtr(1,1),coefd(2,2),coefc(2,2)
      common /coord/ coor(3),coora(27,3)
      x=coor(1)
      rx=refc(1)
      goto (1,2) n
1     fedll3g3=+(+rx*(+rx-1.)/2.)*un(1)+(+(+1.-rx**2))*un(3)
     & +(+rx*(+1.+rx)/2.)*un(2)
      goto 1000
2     fedll3g3=+(+rx*(+rx-1.)/2.)*vn(1)+(+(+1.-rx**2))*vn(3)
     & +(+rx*(+1.+rx)/2.)*vn(2)
      goto 1000
1000  return
      end
