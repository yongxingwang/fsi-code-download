...
c...... get the initial value from the data files by preprocessor
      inquire(file='dispb1',exist=filflg)
      if (filflg) then
      open(16,file='dispb1',form='unformatted',status='old')
      read(16) numnod,noddof,((u0(j,n),j=1,noddof),n=1,numnod)
      close(16)
      endif
      inquire(file='dispb2',exist=filflg)
      if (filflg) then
      open(16,file='dispb2',form='unformatted',status='old')
      read(16) numnod,noddof,((u1(j,n),j=1,noddof),n=1,numnod)
      close(16)
      endif
      inquire(file='dispb3',exist=filflg)
      if (filflg) then
      open(16,file='dispb3',form='unformatted',status='old')
