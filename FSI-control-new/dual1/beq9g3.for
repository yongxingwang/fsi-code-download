...
c .... coefr ---- nodal coef value
     ...
     & eload(18)
      dimension prmt(*),coef(6),coefr(9,6),
...
c .... for all integral points
      common /vbeq9g3/rctr(2,2),crtr(2,2),coefd(6,5),coefc(6,5)
...
c .... crtr ---- inverse matrix of Jacobi's matrix
      ...
      rx=refc(1,igaus)
      ry=refc(2,igaus)
      call ebeq9g3(refc(1,igaus),coef,coorr,coefr,coefd)
...
c .... the following is the load vector computation
      do 501 i=1,9
      iv=kvord(i,1)
      stif=+cu(i,1)*uh*coefc(1,1)*rou
     & +cu(i,1)*vh*coefc(2,1)*rou
      eload(iv)=eload(iv)+stif*weigh
501   continue
      do 502 i=1,9
      iv=kvord(i,2)
      stif=+cv(i,1)*uh*coefc(1,2)*rou
     & +cv(i,1)*vh*coefc(2,2)*rou
      eload(iv)=eload(iv)+stif*weigh
502   continue
999   continue
998   continue
...
c .... kvord ---- var number at integral points for each unknown
     ...
     & coorr(ncoor,nnode),coor(ncoor),coefr(nnode,*)
      call tbeq9g3(refc,coor,coorr,coefr,rc)
      n=nrefc
      m=n*2
      det = 1.0
      do 10 i=1,n
...
c .... shape function caculation
      implicit real*8 (a-h,o-z)
      common /ccbeq9g3/ xa(9),ya(9),una(9),vna(9),
     &usa(9),vsa(9),uha(9),vha(9)
      common /vbeq9g3/ rctr(2,2),crtr(2,2),coefd(6,5),coefc(6,5)
      dimension refc(2)
...
1000  return
      ...
      subroutine tbeq9g3(refc,coor,coorr,coefr,rc)
c .... compute coordinate value and Jacobi's matrix rc
c .... by reference coordinate value
      implicit real*8 (a-h,o-z)
      dimension refc(2),coor(2),coorr(2,9),coefr(9,6),rc(2,2)
      common /ccbeq9g3/ x(9),y(9),un(9),vn(9),us(9),vs(9),
     &uh(9),vh(9)
      external ftbeq9g3
      do 100 n=1,9
      x(n)=coorr(1,n)
...
1000  return
      ...
      subroutine ebeq9g3(refc,coef,coorr,coefr,coefd)
c .... compute coef value and their partial derivatives
c .... by reference coordinate value
      implicit real*8 (a-h,o-z)
      dimension refc(2),coef(6),coorr(2,9),coefr(9,6),coefd(6,2)
      external febeq9g3
      rx=refc(1)
      ry=refc(2)
      call dcoef(febeq9g3,refc,coef,coefd,2,6,2)
...
c .... which is in the file ccshap.for of FEPG library
      ...
      real*8 function febeq9g3(refc,n)
c .... coef function caculation
      implicit real*8 (a-h,o-z)
      dimension refc(2)
      common /ccbeq9g3/ xa(9),ya(9),un(9),vn(9),us(9),vs(9),
     &uh(9),vh(9)
      common /vbeq9g3/ rctr(2,2),crtr(2,2),coefd(6,5),coefc(6,5)
      common /coord/ coor(3),coora(27,3)
      x=coor(1)
      ...
      ry=refc(2)
      goto (1,2,3,4,5,6) n
1     febeq9g3=+(+rx*(+rx-1.)/2.*ry*(+ry-1.)/2.)*un(1)+(+
     ...
     & ry*(+1.+ry)/2.)*un(3)
      goto 1000
2     febeq9g3=+(+rx*(+rx-1.)/2.*ry*(+ry-1.)/2.)*vn(1)+(+
     ...
     & ry*(+1.+ry)/2.)*vn(3)
      goto 1000
3     febeq9g3=+(+rx*(+rx-1.)/2.*ry*(+ry-1.)/2.)*us(1)+(+
     ...
     & ry*(+1.+ry)/2.)*us(3)
      goto 1000
4     febeq9g3=+(+rx*(+rx-1.)/2.*ry*(+ry-1.)/2.)*vs(1)+(+
     ...
     & ry*(+1.+ry)/2.)*vs(3)
      goto 1000
5     febeq9g3=+(+rx*(+rx-1.)/2.*ry*(+ry-1.)/2.)*uh(1)+(+
     & (+1.-rx**2)*ry*(+ry-1.)/2.)*uh(5)+(+rx*(+1.+rx)/2.*ry*
     & (+ry-1.)/2.)*uh(2)+(+rx*(+rx-1.)/2.*(+1.-ry**2))*uh(8)
     & +(+(+1.-rx**2)*(+1.-ry**2))*uh(9)+(+rx*(+1.+rx)/2.*(+
     & 1.-ry**2))*uh(6)+(+rx*(+rx-1.)/2.*ry*(+1.+ry)/2.)*uh(4)
     & +(+(+1.-rx**2)*ry*(+1.+ry)/2.)*uh(7)+(+rx*(+1.+rx)/2.*
     & ry*(+1.+ry)/2.)*uh(3)
      goto 1000
6     febeq9g3=+(+rx*(+rx-1.)/2.*ry*(+ry-1.)/2.)*vh(1)+(+
     & (+1.-rx**2)*ry*(+ry-1.)/2.)*vh(5)+(+rx*(+1.+rx)/2.*ry*
     & (+ry-1.)/2.)*vh(2)+(+rx*(+rx-1.)/2.*(+1.-ry**2))*vh(8)
     & +(+(+1.-rx**2)*(+1.-ry**2))*vh(9)+(+rx*(+1.+rx)/2.*(+
     & 1.-ry**2))*vh(6)+(+rx*(+rx-1.)/2.*ry*(+1.+ry)/2.)*vh(4)
     & +(+(+1.-rx**2)*ry*(+1.+ry)/2.)*vh(7)+(+rx*(+1.+rx)/2.*
     & ry*(+1.+ry)/2.)*vh(3)
      goto 1000
1000  return
      end
