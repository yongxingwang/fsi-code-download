      ...
      kna12=kna12+kna11
      kna13=kna13+kna12
      if (kna13-1.gt.143610000) then
      write(*,*) 'exceed memory of array ia'
      write(*,*) 'memory of ia = 143610000'
      write(*,*) 'memory needed = ',kna13,' in prgram ecgmd'
      stop 55555
      endif
      knb0=1
      knb1=knb1+knb0
      ...
      write(*,*) 'memory needed = ',knb2,' in prgram ecgmd'
      stop 55555
      endif
      call ecgmd(knode,kdgof,kvar,kcoor,
     ...
     *ia(kna12),ib(knb0),ib(knb1),
     *filename)
      end
      subroutine ecgmd(knode,kdgof,kvar,kcoor,
     *numtyp,numel,neq,kelem,lr,u,
     *f,coor,eu,eun,evn,eus,
     *evs,eud,evd,emass,sml,emate,
     *nodvar,node,
     *filename)
      implicit real*8 (a-h,o-z)
      character*12 filename(20)
        dimension nodvar(kdgof,knode),u(kdgof,knode),coor(kcoor,knode),
     *eu(kdgof,knode),eun(knode),evn(knode),eus(knode),
     *evs(knode),eud(knode),evd(knode),emass(kvar),
...
c      write(*,*) 'numnod,ncoor=',numnod,ncoor
      ...
      open(13,file='ugoal',form='unformatted',status='old')
      else
      open(11,file='unod',form='unformatted',status='old')
      open(12,file='unods',form='unformatted',status='old')
      open(13,file='ugoal',form='unformatted',status='old')
...
c.......open unod file and store the solution
     ...
     *eun,evn,eus,evs,eud,evd,eu,emass,
     & u)
      implicit real*8 (a-h,o-z)
      dimension nodvar(kdgof,knode),coor(kcoor,knode),node(kelem),
     & u(kdgof,knode),emate(300),
     *es(k,k),em(k),ef(k),eun(knode),
     *evn(knode),eus(knode),evs(knode),eud(knode),
     *evd(knode),eu(kdgof,knode),estifn(k,k),estifv(kk),
...
18      format (1x,8e9.2)
	...
	jnod = node((ne-1)*nnode+j)
      ...
      coef(j+4*nne)=eud(jnod)
      coef(j+5*nne)=evd(jnod)
	do 120 i=1,ncoor
	nr=nr+1
...
140	prmt(j) = emate((imate-1)*nmate+j)
	...
	prmt(nmate+2)=dt
      ...
      goto 1
1     call deq9g3(r,coef,prmt,es,em,ec,ef,ne)
      goto 2
2     continue
