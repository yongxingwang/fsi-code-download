...
c .... coefr ---- nodal coef value
     ...
     & eload(22)
      dimension prmt(*),
     & ediv(22),coorr(2,9),coor(2)
      common /rfeq9g3/ru(9,27),rv(9,27),rp(4,27),
     & cu(9,3),cv(9,3),cp(4,3)
c .... store shape functions and their partial derivatives
c .... for all integral points
      common /vfeq9g3/rctr(2,2),crtr(2,2)
      common /dfeq9g3/ refc(2,9),gaus(9),
...
10    continue
      ...
      call feq9g3t(nnode,nrefc,ncoor,refc(1,igaus),coor,coorr,
     & rctr,crtr,det)
...
c .... the following is the shape function caculation
      call feq9g31(refc(1,igaus),ru(1,iu),rctr,crtr)
      call feq9g32(refc(1,igaus),rv(1,iv),rctr,crtr)
      call feq9g33(refc(1,igaus),rp(1,ip),rctr,crtr)
2     continue
...
8     format(1x,6f12.3)
      ...
      subroutine feq9g31(refc,shpr,rctr,crtr)
...
c .... shapr ---- store shape functions and their partial derivatives
      ...
      rx=refc(1)
      ry=refc(2)
      call dshap(ffeq9g31,refc,shpr,2,9,1)
...
c .... which is in the file ccshap.for of FEPG library
      ...
      real*8 function ffeq9g31(refc,n)
c .... shape function caculation
      implicit real*8 (a-h,o-z)
      common /ccfeq9g3/ xa(9),ya(9)
      common /vfeq9g3/ rctr(2,2),crtr(2,2)
      dimension refc(2)
...
1     ffeq9g31=+rx*(+rx-1.)/2.*ry*(+ry-1.)/2. 
      goto 1000
2     ffeq9g31=+(+1.-rx**2)*ry*(+ry-1.)/2. 
      goto 1000
3     ffeq9g31=+rx*(+1.+rx)/2.*ry*(+ry-1.)/2. 
      goto 1000
4     ffeq9g31=+rx*(+rx-1.)/2.*(+1.-ry**2) 
      goto 1000
5     ffeq9g31=+(+1.-rx**2)*(+1.-ry**2) 
      goto 1000
6     ffeq9g31=+rx*(+1.+rx)/2.*(+1.-ry**2) 
      goto 1000
7     ffeq9g31=+rx*(+rx-1.)/2.*ry*(+1.+ry)/2. 
      goto 1000
8     ffeq9g31=+(+1.-rx**2)*ry*(+1.+ry)/2. 
      goto 1000
9     ffeq9g31=+rx*(+1.+rx)/2.*ry*(+1.+ry)/2. 
      goto 1000
1000  return
      end

      subroutine feq9g32(refc,shpr,rctr,crtr)
c .... compute shape functions and their partial derivatives
c .... shapr ---- store shape functions and their partial derivatives
      implicit real*8 (a-h,o-z)
      dimension refc(2),shpr(9,3),rctr(2,2),crtr(2,2)
      ...
      rx=refc(1)
      ry=refc(2)
      call dshap(ffeq9g32,refc,shpr,2,9,1)
c .... shape function and their derivatives computation
c .... compute partial derivatives by centered difference
c .... which is in the file ccshap.for of FEPG library
      return
      ...
      real*8 function ffeq9g32(refc,n)
c .... shape function caculation
      implicit real*8 (a-h,o-z)
      common /ccfeq9g3/ xa(9),ya(9)
      common /vfeq9g3/ rctr(2,2),crtr(2,2)
      ...
      ry=refc(2)
      goto (1,2,3,4,5,6,7,8,9) n
1     ffeq9g32=+rx*(+rx-1.)/2.*ry*(+ry-1.)/2. 
      goto 1000
2     ffeq9g32=+(+1.-rx**2)*ry*(+ry-1.)/2. 
      goto 1000
3     ffeq9g32=+rx*(+1.+rx)/2.*ry*(+ry-1.)/2. 
      goto 1000
4     ffeq9g32=+rx*(+rx-1.)/2.*(+1.-ry**2) 
      goto 1000
5     ffeq9g32=+(+1.-rx**2)*(+1.-ry**2) 
      goto 1000
6     ffeq9g32=+rx*(+1.+rx)/2.*(+1.-ry**2) 
      goto 1000
7     ffeq9g32=+rx*(+rx-1.)/2.*ry*(+1.+ry)/2. 
      goto 1000
8     ffeq9g32=+(+1.-rx**2)*ry*(+1.+ry)/2. 
      goto 1000
9     ffeq9g32=+rx*(+1.+rx)/2.*ry*(+1.+ry)/2. 
      goto 1000
1000  return
      end

      subroutine feq9g33(refc,shpr,rctr,crtr)
c .... compute shape functions and their partial derivatives
c .... shapr ---- store shape functions and their partial derivatives
      implicit real*8 (a-h,o-z)
      ...
      rx=refc(1)
      ry=refc(2)
      call dshap(ffeq9g33,refc,shpr,2,4,1)
c .... shape function and their derivatives computation
c .... compute partial derivatives by centered difference
c .... which is in the file ccshap.for of FEPG library
      return
      ...
      real*8 function ffeq9g33(refc,n)
c .... shape function caculation
      implicit real*8 (a-h,o-z)
      common /ccfeq9g3/ xa(9),ya(9)
      common /vfeq9g3/ rctr(2,2),crtr(2,2)
      ...
      ry=refc(2)
      goto (1,2,3,4) n
1     ffeq9g33=+(+1.-rx)/2.*(+1.-ry)/2. 
      goto 1000
2     ffeq9g33=+(+1.+rx)/2.*(+1.-ry)/2. 
      goto 1000
3     ffeq9g33=+(+1.+rx)/2.*(+1.+ry)/2. 
      goto 1000
4     ffeq9g33=+(+1.-rx)/2.*(+1.+ry)/2. 
      goto 1000
1000  return
      end

      subroutine tfeq9g3(refc,coor,coorr,rc)
c .... compute coordinate value and Jacobi's matrix rc
c .... by reference coordinate value
      implicit real*8 (a-h,o-z)
      dimension refc(2),coor(2),coorr(2,9),rc(2,2)
      common /ccfeq9g3/ x(9),y(9)
      external ftfeq9g3
      do 100 n=1,9
      x(n)=coorr(1,n)
      y(n)=coorr(2,n)
100   continue
      rx=refc(1)
      ry=refc(2)
      call dcoor(ftfeq9g3,refc,coor,rc,2,2,1)
...
c .... which is in the file ccshap.for of FEPG library
      ...
      real*8 function ftfeq9g3(refc,n)
c .... coordinate transfer function caculation
      ...
      ry=refc(2)
      goto (1,2) n
1     ftfeq9g3=+(+rx*(+rx-1.)/2.*ry*(+ry-1.)/2.)*x(1)+(+(+
     ...
     & (+1.+ry)/2.)*x(3)
      goto 1000
2     ftfeq9g3=+(+rx*(+rx-1.)/2.*ry*(+ry-1.)/2.)*y(1)+(+(+
