...
c.......open sys file
     ...
     *ic(knc12),ic(knc13),ic(knc14),ic(knc15),
...
c...will not be used after ascembling	 
     ...
     * eun(knode),evn(knode),epn(knode),node(kelem),sml(100000)
...
c.......open id0 file
      open (22,file='id0',form='unformatted',status='old')
...
c... g will be used as a temporary array
     ...
     &          b,nb,numcb,jd,dd,g,tg,g1,tg1,nodvar,id0,u,
...
3200  continue
     ...
     & ityp,ncoor,num,time,dt,neq,neq1,maxt,maxa,nodvar,coor,node,emate,
     & a,na,numcol,jdiag,
     *es,em,ef,estifn,estifv,eun,evn,epn,
     *eu0,eu,u)
      implicit real*8 (a-h,o-z)
     ...
     *evn(knode),epn(knode),eu0(kdgof,knode),
...
cccccccccccccccc      write(*,*) 'iter,err = ',iter,err
