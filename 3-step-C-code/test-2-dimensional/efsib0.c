...
*/
#include "fsi.h"
void b0eq9g3(double *,double *,double *,double *,double *,double *,double *,int);
/*  adda(): assembly for global matrix  */
void adda(int *,double *,int *,int,int *,double *,int,int);
void efsib0(coor0,dof,nodvar,ubf,elem,matrix,f)
int dof,*nodvar;
double *ubf,*f;
...
struct matrice matrix;
{
    ...
    */
    ...
    coor  = coor0.coor;
    kvar  = knode*dof;
    vectun = (double *) calloc(kvar,sizeof(double));
    a = matrix.a;
    na = matrix.na;
    neq = matrix.neq;
    maxa = matrix.maxa;
    ...
    for (j=1; j<=dof; ++j)
        for (i=1; i<=knode; ++i)
            vectun[(j-1)*(knode)+i-1] = unodc[(nrw+j-1)*knode+i-1];
    nrw += dof;
    numel = 0;
    nn = 0;
    mm = 0;
    ...
    for (ityp=1; ityp<=ntype; ++ityp)
    {
        ...
        for(ne=1; ne<=nelem; ++ne)
        {
            ...
            for (j=1; j<=nne; ++j)
            {
                ...
                for (l=1; l<=dof; ++l)
                {
                    coef[j-1+i*nne]=vectun[(l-1)*(knode)+jnod-1];
                    i++;
                }
                for(i=1; i<=dim; ++i)
            ...
            {
            case 1 :
                b0eq9g3(r,coef,prmt,es,em,ec,ef,ne);
                break;
            }
            ...
            for (inod=1; inod<=nne; ++inod)
            {
                ...
                for (idof=1; idof<=dof; ++idof)
                {
                    ...
                    if(inv<0)
                    {
                        es_count += 1;
                        for (ij=1; ij<=kk; ++ij)
                            esb[(es_count-1)*kk+ij-1]=estifn[ij-1];
                        goto stored;
                    }
                }
            }
