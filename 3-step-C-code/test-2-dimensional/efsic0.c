...
void efsic0(coor0,dof,nodvar,ubf,elem,matrix,f)
int dof,*nodvar;
double *ubf,*f;
    ...
    */
    ...
    for (ityp=1; ityp<=ntype; ++ityp)
    {
        ...
        for(ne=1; ne<=nelem; ++ne)
        {
            ...
            {
            case 1 :
                ceq9g3(r,coef,prmt,es,em,ec,ef,ne);
                break;
            }
            ...
            for (inod=1; inod<=nne; ++inod)
            {
                ...
                for (idof=1; idof<=dof; ++idof)
                {
                    ...
                    if(inv<0)
                    {
                        es_count += 1;
                        for (ij=1; ij<=kk; ++ij)
                            esc[(es_count-1)*kk+ij-1]=estifn[ij-1];
                        goto stored;
                    }
                }
            }
...
////////////////////////////////////////////////////////////////////////
            ...
            for (inod=1; inod<=nne; ++inod)
            {
                ...
                for (idof=1; idof<=dof; ++idof)
                {
                    ...
                    if  (inv>0)
                        emc[(idof-1)*(knode)+nodi-1] += em[i]/dt;
////////////////////////////////////////////////////////////////////////////////////////
