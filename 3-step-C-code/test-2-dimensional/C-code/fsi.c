...
void efsib0(struct coordinates,int,int *,double *,struct element,struct matrice,double *);
...
int main ()
{
    ...
    printf("%s\n","efsid()...preconditioner for pressure step");
    efsid(coor0,dofc,idc,ubfc,elem0,matrixc,fc);
    for (i=1; i<=matrixc.maxa; ++i) (matrixc.p)[i]=(matrixc.a)[i];

    printf("%s\n","cholesky(matrixc)...preconditioner for pressure step");
    cholesky(matrixc);

    printf("%s\n","efsib0()...preconditioner for convection");
    efsib0(coor0,dofa,ida,ubfa,elem0,matrixb,fa);
    for (i=1; i<=matrixb.maxa; ++i) (matrixb.p)[i]=(matrixb.a)[i];

    printf("%s\n","cholesky(matrixb)...preconditioner for convection");
    cholesky(matrixb);

    printf("%s\n","efsia0()...preconditioner for diffusoin");
    efsia0(coor0,dofa,ida,ubfa,elem0,matrixa,fa);
    for (i=1; i<=matrixa.maxa; ++i) (matrixa.p)[i]=(matrixa.a)[i];

    printf("%s\n","cholesky(matrixa)...preconditioner for diffusion");
    cholesky(matrixa);
...
//    end_time = clock();
//    cpu_time = ((double) (end_time - start_time)) / CLOCKS_PER_SEC /3.6e3;
//    printf("%f hours are used \n", cpu_time);


    // compute the cup time to check whether it's time to restart program //
    // used when submitting jobs on ARC //
    /*
        if (cpu_time > 47. && stop==0)
        {
            fp = fopen("restart","w");
            fclose(fp);
