...
*/
#include "fsi.h"
void fet3(double *,double *,double *,double *,double *,double *,double *,int);
/*  adda(): assembly for global matrix  */
void adda(int *,double *,int *,int,int *,double *,int,int);
void efsif(coor0,dof,nodvar,ubf,elem,matrix,f)
int dof,*nodvar;
double *ubf,*f;
...
struct matrice matrix;
{
    ...
    */
    ...
    for (j=1; j<=dof; ++j)
        for (i=1; i<=knode; ++i)
            vectu0[(j-1)*(knode)+i-1] = unodf[(nrw+j-1)*knode+i-1];
    nrw += dof;
    numel = 0;
    nn = 0;
    mm = 0;
    ...
    for (ityp=1; ityp<=ntype; ++ityp)
    {
        ...
        for(ne=1; ne<=nelem; ++ne)
        {
            ...
            {
            case 1 :
                fet3(r,coef,prmt,es,em,ec,ef,ne);
                break;
            }
            ...
            for (inod=1; inod<=nne; ++inod)
            {
                ...
                for (idof=1; idof<=dof; ++idof)
                {
                    ...
                    if  (inv>0)
                    {
                        u[(idof-1)*knode+nodi-1] += ef[i]+em[i]*vectu0[(idof-1)*knode+nodi-1]/dt;
                    }
