...
*/
#include "fsi.h"
void ceq9g3(double *,double *,double *,double *,double *,double *,double *,int);
/*  adda(): assembly for global matrix  */
void adda(int *,double *,int *,int,int *,double *,int,int);
void efsic(coor0,dof,nodvar,ubf,elem,matrix,f)
int dof,*nodvar;
double *ubf,*f;
...
struct matrice matrix;
{
    ...
    */
    ...
    int dim,knode,neq,*numcol,maxa,*na,node[500];
    ...
    coor  = coor0.coor;
    kvar  = knode*dof;
    vectu0 = (double *) calloc(kvar,sizeof(double));
//    a = matrix.a;
    na = matrix.na;
    neq = matrix.neq;
    maxa = matrix.maxa;
    ...
    */
    ...
    for (j=1; j<=dof; ++j)
        for (i=1; i<=knode; ++i)
            vectu0[(j-1)*knode+i-1] = unodc[(nrw+j-1)*knode+i-1];
    nrw += dof;
    numel = 0;
    nn = 0;
    mm = 0;
...
////////////////////////////////////////////////////////////////////
//read the element stiff matrix in ordr to treat constraint
            for (inod=1; inod<=nne; ++inod)
            {
                ...
                for (idof=1; idof<=dof; ++idof)
                {
                    ...
                    if(inv<0)
                    {
                        ...
                        for (ij=1; ij<=kk; ++ij)
                        {
                            estifn[ij-1]=esc[(es_count-1)*kk+ij-1];
                        }
                        goto stored;
                    }
                }
        ...
        for (j=1; j<=knode; ++j)
        {
            ...
            if  (ij>0)
                f[ij] += u[(i-1)*knode+j-1]
                         +emc[(i-1)*knode+j-1]*vectu0[(i-1)*knode+j-1];
        }
    }
    free(u);
    free(vectu0);
//      printf("the end of efsic():%d\n",nodvar[(1-1)*(knode)+259567-1]);
//        scanf("%d",&i);
