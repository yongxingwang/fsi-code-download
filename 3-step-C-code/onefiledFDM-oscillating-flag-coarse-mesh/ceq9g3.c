...
/* subroutine */
void ceq9g3(coora,coefa,prmt,estif,emass,edamp,eload,num)
...
// .... the following is the stiffness computation
        for (iv=1; iv<=22; ++iv)
        {
            ...
            {
                jv=kvord[(j-1)*(3)+3-1];
                stif=-ediv[iv]*cp[(j-1)*(3)+1-1] ;
                estif[(iv-1)*(22)+jv-1]=estif[(iv-1)*(22)+jv-1]+stif*weigh;
            }
        }
        for (i=1; i<=4; ++i)
        {
