...
l2:
        ...
        {
            iv=kvord[(i-1)*(2)+1-1];
            stif=+cu[(i-1)*(3)+1-1]*gxx+cu[(i-1)*(3)+2-1]*un*dt+cu[(i-1)*(3)+3-1]*vn*dt;
            elx[iv]+=stif;
        }
        for (i=1; i<=9; ++i)
        {
            iv=kvord[(i-1)*(2)+2-1];
            stif=+cv[(i-1)*(3)+1-1]*gxy;
            elx[iv]+=stif;
        }
        for (i=1; i<=9; ++i)
        {
            iv=kvord[(i-1)*(2)+1-1];
            stif=+cu[(i-1)*(3)+1-1]*gyx;
            ely[iv]+=stif;
        }
        for (i=1; i<=9; ++i)
        {
            iv=kvord[(i-1)*(2)+2-1];
            stif=+cv[(i-1)*(3)+1-1]*gyy+cv[(i-1)*(3)+2-1]*un*dt+cv[(i-1)*(3)+3-1]*vn*dt;
            ely[iv]+=stif;
        }
// .... the following is the stiffness computation
        for (iv=1; iv<=18; ++iv)
        {
            for (jv=1; jv<=18; ++jv)
            {
                stif=+elx[iv]*elx[jv]
                     +ely[iv]*ely[jv] ;
                estif[(iv-1)*(18)+jv-1]=estif[(iv-1)*(18)+jv-1]+stif*weigh;
            }
        }
